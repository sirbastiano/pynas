...
class GenericNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx, layer in enumerate(self.layers):
            #print(f"Layer {idx} - {layer.__class__.__name__}: Input shape: {x.shape}")

            if idx in self.skip_connections:
                skip_data[idx] = x.clone()  # Store the current state

            x = layer(x)

            if idx in self.skip_indices:
                skip_from_idx = list(self.skip_connections.keys())[list(self.skip_connections.values()).index(idx)]
                #print(f"Adding skip connection from layer {skip_from_idx} to layer {idx}")
                skip_tensor = skip_data[skip_from_idx]

                # Check if the channels and spatial dimensions match
                if x.shape != skip_tensor.shape:
                    #print(f"Shape mismatch before adjustment: {x.shape} vs {skip_tensor.shape}")

                    # Adjust channels if needed using a 1x1 convolution layer
                    if x.shape[1] != skip_tensor.shape[1]:
                        adjust_channels = nn.Conv2d(skip_tensor.shape[1], x.shape[1], kernel_size=1).to(x.device)
                        skip_tensor = adjust_channels(skip_tensor)

                    # Adjust spatial dimensions if needed (you can use interpolation for this)
                    if x.shape[2:] != skip_tensor.shape[2:]:
                        skip_tensor = F.interpolate(skip_tensor, size=(x.shape[2], x.shape[3]), mode='nearest')

                    #print(f"Shape mismatch after adjustment: {x.shape} vs {skip_tensor.shape}")

                x = x + skip_tensor  # Add skip connection

            #print(f"Layer {idx} - {layer.__class__.__name__}: Output shape: {x.shape}")
