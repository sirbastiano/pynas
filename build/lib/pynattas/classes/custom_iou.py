...
def calculate_iou(logits, targets, num_classes=4):
    with torch.no_grad():
        ...
        for cls in range(num_classes):
            intersection = ((preds == cls) & (targets == cls)).float().sum((1, 2))
            union = ((preds == cls) | (targets == cls)).float().sum((1, 2))
            iou.append((intersection + 1e-6) / (union + 1e-6))
        return torch.stack(iou).mean(dim=0)  # Mean IoU across all classes
